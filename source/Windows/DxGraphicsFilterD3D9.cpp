...
static int	Direct3D9_GraphFilter_GetWorkTexture( int IsFloatType, DWORD TexSizeW, DWORD TexSizeH, DWORD HandleIndex )
{
	...
	if( GRAPHCHK( GraphFilterSystemInfoD3D9.WorkDrawValidGrHandle[ IsFloatType ][ NPow ][ HandleIndex ], BaseImage ) )
	{
		...
		GraphFilterSystemInfoD3D9.WorkDrawValidGrHandle[ IsFloatType ][ NPow ][ HandleIndex ] = Graphics_Image_MakeGraph_UseGParam( &GParam, TexSize, TexSize, FALSE, FALSE, 0, FALSE ) ;
...
static int Direct3D9_MemLoadShaderCode( const char *ShaderName, bool IsVertexShader )
{
	...
	if( DXA_GetFileInfo( &GD3D9.ShaderCode._2D.FilterShaderBinDxa, DX_CHARCODEFORMAT_ASCII, ShaderName, &Addr, &Size ) < 0 ) return -1 ;
...
extern int	GraphFilter_D3D9_Bright_Scale_PF( GRAPHFILTER_INFO *Info, int BrightMin, int BrightMax )
{
	...
	{
		GraphFilterShaderHandle.BrightScalePS = Direct3D9_MemLoadShaderCode( PsoFileName[ 0 ], false ) ;
		if( GraphFilterShaderHandle.BrightScalePS < 0 )
		{
			char PathUTF16LE[ 128 ] ;
	...
	NS_SetUsePixelShader( GraphFilterShaderHandle.BrightScalePS ) ;
