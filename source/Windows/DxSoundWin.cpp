...
R1 :
		...
		if( SoundSysData.PF.DirectSoundObject )
		{
			...
			{
				DXST_ERRORLOG_ADDUTF16LE( "\x44\x00\x53\x00\x4f\x00\x55\x00\x4e\x00\x44\x00\x2e\x00\x44\x00\x4c\x00\x4c\x00\x20\x00\x6e\x30\xed\x30\xfc\x30\xc9\x30\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"SoundSysData.DLL のロードに失敗しました\n" @*/ ) ;
				DXST_ERRORLOG_TABSUB ;
				DXST_ERRORLOG_TABSUB ;
				goto ENUMEND ;
			}
...
extern int NS_SetBeepFrequency( int Freq )
{
	...
	// データをセットする
	{
		...
		hr = SoundSysData.PF.BeepSoundBuffer->Lock( 0 , dsbdesc.dwBufferBytes, &write1 , &length1 , &write2 , &length2 , 0 ) ;		// バッファのロック
	...
	SoundSysData.PF.BeepSoundBuffer->Unlock( write1 , length1 , write2 , length2 ) ;								// バッファのロック解除
