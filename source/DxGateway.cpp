...
extern int DrawRoundRectAA( float x1, float y1, float x2, float   y2, float rx, float ry, unsigned int Color, int FillFlag, float LineThickness )
{
	int Result ;
	DXFUNC_BEGIN
	Result = NS_DrawRoundRectAA(  x1,  y1,  x2,  y2,  rx,  ry,  Color,  FillFlag, LineThickness ) ;
	DXFUNC_END
	return Result ;
}
...
extern	int		MV1GetTriangleListPolygonVertexPosition( int MHandle, int TListIndex, int PolygonIndex, VECTOR *VertexPositions )
{
	int Result ;
	DXFUNC_BEGIN
	Result = NS_MV1GetTriangleListPolygonVertexPosition( MHandle, TListIndex, PolygonIndex, VertexPositions ) ;
	DXFUNC_END
	return Result ;
}
